        ...
        try:
            ...
            with get_connection() as conn:
                with conn.cursor() as cur:
                    # Does player exist?
                    cur.execute(
                        """SELECT * FROM players WHERE game_id=%(game_id)s;""",
                        {"game_id": game_id},
                    )
                    results = cur.fetchall()

            print(f"Selected players: {results}")
            players = []
            ...
            logging.info(f"Selected players with game_id: {game_id}")
